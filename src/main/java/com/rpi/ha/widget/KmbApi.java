...
/***
 * Modified KmbApi from Repository KmbApi
 * @author Anthony
 *
 */
public class KmbApi {
	
	private static final Logger logger = LogManager.getLogger(KmbApi.class.getName());
	...
	public static JSONObject etaJsonData(String route, String busStopCode, int lang, int bound, int stop_seq)
	{
		try {
			...
			logger.error("Could not connect to ETA server.");
			return null;
		}
	}
	...
	public static String getServerTime(){
		try {
			...
			logger.error("Could not connect to Data Feed Server");
	...
	*/
	...
	public static int getStopSeq(String route, String stopcode, int bound){
		String[] data;
		for (int i = 0; i < busstop_pair.toArray().length; i++){
			data = busstop_pair.get(i);
			if (data[0].equals(route) && data[1].equals(Integer.toString(bound)) && data[2].equals(stopcode)){
				System.out.println("R: " + route + " Stopcode: " + stopcode + "B: " + bound + " Stopseq: " + data[3]);
	...
	public static List<String[]> getAllETAtableData(int lang, String stopcode){
		...
		for (int i = 0; i < stopbuses.size(); i++){
			...
			JSONObject etajson = etaJsonData(data[0], stopcode, lang, Integer.parseInt(data[1]), Integer.parseInt(data[3]));
			if (etajson == null){
				build[0] = data[0];
				build[1] = "---";
				build[2] = "---";
